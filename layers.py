...
class Embedding(torch.nn.Module):
    ...
    def __init__(self, embedding_size, vocab_size, dropout_rate=0.0, trainable=True, id2word=None,
        ...
        self.embedding_layer = torch.nn.Embedding(self.vocab_size, self.embedding_size, padding_idx=0) #padding_ind
        self.init_weights()
    ...
    def compute_mask(self, x):
        mask = torch.ne(x, 0).float() # 0 is the placeholder of padding word?
        if x.is_cuda:
            mask = mask.cuda()
        return mask
    ...
    def forward(self, x):
        ...
        mask = self.compute_mask(x)  # batch x time -->mask for padded 
